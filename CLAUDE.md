...
| デプロイ | **GCP（Cloud Run + Cloud SQL + Firebase Hosting）**。選定根拠は `docs/DOC-IF-001_cloud_selection_gcp_aws.md`、手順は `docs/gcp-deploy.md`。※リテール事業系はGCP方針（`aws` スキルは fieldup 用であり本プロジェクトでは使わない） |
