...
| `Unit` | `PartUnit` | Identity = `serial_number`。件数が多いため UI は 製品→部品コード→実物一覧 のドリルダウン（`?part_master=<id>` でフレーム切替）。部品コード一覧は `PartMaster` に annotate した `unit_count`/`in_stock_count`/`broken_count` を表示。現在の場所 = 搭載中なら `current_set`（BssSetComponent 由来）／未搭載なら `storage_site`（**倉庫**）。取外しは セット詳細の取外しドロワーで ステータス＋保管先倉庫＋保守イベント を一括更新 |
...
6. **倉庫（ロケーション）は CustomerSite を再利用**：外した部品の保管先「倉庫」は新モデルを作らず、`lifecycle_status=拠点(BASE)` の `CustomerSite`（＝AIBOD自身の拠点）を倉庫として扱う（AIBOD大名/大熊オフィスが該当）。`PartUnit.storage_site` が未搭載時の保管先。**設置先（顧客の拠点）と保管先（倉庫）を同じ CustomerSite で統一**し、場所概念を二重化しない（別 `StorageLocation` は作らない）。UIの「倉庫」は BASE拠点の絞り込みビュー。
